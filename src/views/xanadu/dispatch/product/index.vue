        ...
        <el-tabs tab-position="left" v-model="activeName" @tab-click="changeTab" style="height: 100%;">
            ...
            <el-tab-pane label="分库调度入库" name="sub">
                ...
                <div class="alert">
                    ...
                    <p v-else>正在调拨的商品编号为:<code>{{ productId }}</code>,可分配商品数量为:<code>{{ centerStorage }}</code>,请选择分库,完成商品调度</p>
                </div>
                <el-table :key="1" :row-key="(row) => row.id" :data="subList" ref="table" border fit highlight-current-row
...
export default {
    ...
    components: { Pagination,dispatchRecords },
    data() {
        return {
            listLoading: false,
            centerList: [],
