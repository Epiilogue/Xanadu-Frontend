...
import { cenGoodById, centerStorageList, subWareList } from '@/api/ware.js'
...
export default {
  ...
  methods: {
    refreshList() {
      if (this.productName !== '') {
        this.centerList = this.centerList.filter(item => item.productName.indexOf(this.productName) > -1)
      } else {
        this.productName = ''
        this.getCenterList()
      }
    ...
    submit() {
      ...
      getProduct(this.req.product.productId).then(res => {
        ...
        this.req.product.productCategary = res.data.firstName
